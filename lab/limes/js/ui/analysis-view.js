function analyze(){
 const raw=document.getElementById("txt").value.trim();const issue=getStrategicIssue();if(!raw){document.getElementById("out").innerHTML='<div class="card">Ingrese un texto para analizar.</div>';return;}
 ...
 <div class="card ${issue?"":"epistemic-warning"}"><b>Cuestión estratégica</b><span class="concept-note">${issue?`x se interpreta exclusivamente respecto de: <b>${escapeHtml(issue)}</b>.`:"No se declaró una cuestión estratégica. El análisis semiótico y de actores continúa, pero Limes no debe convertir ausencia de cuestión en una posición neutral."}</span></div>
 ...
 <div class="card"><b>Actores detectados</b><span class="concept-note">Entidades reconocidas en el texto y convertidas en unidades estratégicas. La estimación incorpora acciones emitidas, acciones recibidas y tipo de relación.</span>${detectedActors.length?detectedActors.map(a=>`<div class="actor-row"><span><b>${escapeHtml(a.name)}</b><br><span class="muted">${escapeHtml(a.contexts[0]||"")}</span><small>Posición x: ${Number.isFinite(a.x)?a.x.toFixed(1):"?"} · confianza ${a.xConfidence.toFixed(2)} · ${escapeHtml(a.xMethod)}<br>Valencia v: ${a.v>=0?"+":""}${a.v.toFixed(2)} · evidencia léxica ${a.valenceEvidence}<br>Relaciones: ${a.relations.outgoing.length} emitidas · ${a.relations.incoming.length} recibidas · conflicto ${a.relations.conflict} · cooperación ${a.relations.cooperation}</small><div class="state-grid">${stateCards(a.states,true)}</div></span><span class="badge">${a.type}</span><span class="badge">${a.count} mención${a.count===1?"":"es"}</span></div>`).join(""):"—"}</div>
...
function transferActors(){
 if(!detectedActors.length)return;
 const tbody=document.querySelector("#actors tbody");
 tbody.innerHTML=detectedActors.map(a=>`<tr data-x-confidence="${a.xConfidence||0}" data-x-method="${escapeHtml(a.xMethod||"")}" data-inferred="${Number.isFinite(a.x)?"true":"false"}"><td><input value="${escapeHtml(a.name)}"></td><td><input class="${Number.isFinite(a.x)?"":"unknown-input"}" type="number" placeholder="?" value="${Number.isFinite(a.x)?a.x:""}"></td><td><input type="number" step="0.01" min="-1" max="1" value="${a.v.toFixed(2)}"></td><td><input type="number" value="${a.c}"></td><td><input type="number" step="0.1" value="${a.s}"></td><td><input type="number" step="0.1" value="${a.r}"></td><td><input type="number" step="0.1" value="${a.rho}"></td><td><input type="number" step="0.1" min="0" value="${a.uncertainty}"></td><td><input type="number" step="0.01" value="${a.states.ser.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.estar.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.decir.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.hacer.toFixed(3)}"></td></tr>`).join("");
 focalActorId=0;refreshFocalSelector();
 const issue=getStrategicIssue();
 const known=readActors().length;
 if(issue&&known>=2)simulate();
 else{
  lastEngine=null;
  document.getElementById("sim").innerHTML=`<div class="card epistemic-warning"><b>Simulación en espera</b><span class="concept-note">${!issue?"Declare una cuestión estratégica. ":""}${known<2?"Se requieren al menos dos actores con x conocido; complete manualmente los campos “?” o aporte relaciones suficientes.":""}</span></div>`;
 }
}


function getStrategicIssue(){return document.getElementById("issueInput")?.value.trim()||"";}
