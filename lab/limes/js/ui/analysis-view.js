function analyze(){
 ...
 <div class="card"><b>Actores detectados</b><span class="concept-note">Entidades reconocidas en el texto y convertidas en unidades estratégicas. La estimación incorpora acciones emitidas, acciones recibidas y tipo de relación.</span>${detectedActors.length?detectedActors.map(a=>`<div class="actor-row"><span><b>${escapeHtml(a.name)}</b><br><span class="muted">${escapeHtml(a.contexts[0]||"")}</span><small>Posición x: ${a.x.toFixed(1)} · confianza ${a.xConfidence.toFixed(2)} · ${escapeHtml(a.xMethod)}<br>Valencia v: ${a.v>=0?"+":""}${a.v.toFixed(2)} · evidencia léxica ${a.valenceEvidence}<br>Relaciones: ${a.relations.outgoing.length} emitidas · ${a.relations.incoming.length} recibidas · conflicto ${a.relations.conflict} · cooperación ${a.relations.cooperation}</small><div class="state-grid">${stateCards(a.states,true)}</div></span><span class="badge">${a.type}</span><span class="badge">${a.count} mención${a.count===1?"":"es"}</span></div>`).join(""):"—"}</div>
...
function transferActors(){if(!detectedActors.length)return;const tbody=document.querySelector("#actors tbody");tbody.innerHTML=detectedActors.map(a=>`<tr><td><input value="${escapeHtml(a.name)}"></td><td><input type="number" value="${a.x}"></td><td><input type="number" step="0.01" min="-1" max="1" value="${a.v.toFixed(2)}"></td><td><input type="number" value="${a.c}"></td><td><input type="number" step="0.1" value="${a.s}"></td><td><input type="number" step="0.1" value="${a.r}"></td><td><input type="number" step="0.1" value="${a.rho}"></td><td><input type="number" step="0.1" min="0" value="${a.uncertainty}"></td><td><input type="number" step="0.01" value="${a.states.ser.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.estar.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.decir.toFixed(3)}"></td><td><input type="number" step="0.01" value="${a.states.hacer.toFixed(3)}"></td></tr>`).join("");focalActorId=0;refreshFocalSelector();simulate();}
