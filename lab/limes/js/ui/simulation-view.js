function renderSimulation(result){
 ...
 <div class="metric-line"><span class="metric-label">Posición x</span><span class="metric-value">${focalInput.x.toFixed(2)} → ${focalFinal.x.toFixed(2)}</span><span class="metric-help">Coordenada estratégica relativa; desplazamiento neto ${(focalFinal.x-focalInput.x).toFixed(2)}.</span></div>
 <div class="metric-line"><span class="metric-label">Valencia v</span><span class="metric-value">${focalInput.v>=0?"+":""}${focalInput.v.toFixed(2)}</span><span class="metric-help">Tonalidad léxica observada; en v0.35 no interviene directamente en la dinámica de x.</span></div>
...
function drawEngine(A,conv,history){
 ...
 svg.innerHTML+=`<text x="4" y="${svgHeight-8}" fill="#6B7568" font-family="ui-monospace, SF Mono, Consolas, monospace" font-size="9" font-weight="600">0 · polo relacional A</text>`;
 svg.innerHTML+=`<text x="496" y="${svgHeight-8}" fill="#6B7568" font-family="ui-monospace, SF Mono, Consolas, monospace" font-size="9" font-weight="600" text-anchor="end">polo relacional B · 100</text>`;
 A.forEach((a,i)=>{
