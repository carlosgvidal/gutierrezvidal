...
(function(global){
  ...
  function validateActor(a){
    ...
    for(const k of ["S","E","H"]){if(!Number.isFinite(a[k])||a[k]<0||a[k]>1)throw new Error(`Actor ${a.id}: ${k} inválido`);}
  }
  function validateOperation(o){
    if(!o||!o.source||!o.target)throw new Error("Operación sin source/target");
    for(const k of ["D","phi","G"]){if(!Number.isFinite(o[k])||o[k]<0||o[k]>1)throw new Error(`Operación ${o.source}→${o.target}: ${k} inválido`);}
  }
  ...
  function runSequence(actors,ops){
    const state=new Map();actors.forEach(a=>{validateActor(a);state.set(a.id,{...a});});
    const history=[];
